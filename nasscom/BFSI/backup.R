...
#a_train = data.table(a_train,
#                     key = "Identifier")
#a_train<-a_train[!c(923,8214,7237,6732,7004),]

X_target1 <- as.numeric(a_train$Application_Score)[1:6924]
X_target2 <- as.numeric(a_train$Behavioural_Score)[1:6924]
...
X_train <- a_train[,":="(Application_Score=NULL,Behavioural_Score=NULL,
                         Salary_Amount=Data.imp$Salary_Amount,
                         Credit_Limit=Data.imp$Credit_Limit,
                         `Pre-Approved Mortgage Limit`=Data.imp$`Pre-Approved Mortgage Limit`,
                         `Pre-Approved Auto Limit`=Data.imp$`Pre-Approved Auto Limit`,
...
m1<-lm(X_target1~.-(Identifier+Customer_Segment+Indutry_Groups +Gender+
                        Insurance_Product_type+Residence+Active_Bank_Products+
                        Active_Bank_Products +Credit_Card +Salary_Amount
                    +Personal_Loan+Mortgage_Loan ),data = X_train)
