...
tr<-fread("BFSI Stage 1 Train data.csv",stringsAsFactors=TRUE)
te<-fread("BFSI Stage 1 Test data.csv",stringsAsFactors=TRUE)
sol<-fread("BFSI - Solution submission template.csv",stringsAsFactors=TRUE)
...
a$`Pre-Approved Personal Limit`<-ordered(a$`Pre-Approved Personal Limit`,levels=c("MISSING ","ZERO","<1,500,000","<1,000,000","<500,000"),labels=c(NA,0,3,2,1))
...
a[,c("Industry_Domain","Metropolitan_City"):=NULL]
a$Gender<-as.numeric(a$Gender)-1
